...
#ifndef SATELLITE_LEO_NET_DEVICE_H
#define SATELLITE_LEO_NET_DEVICE_H
...
/**
 * \ingroup satellite
 * \brief SatLeoNetDevice to be utilized in geostationary satellite.
 * SatLeoNetDevice holds a set of phy layers towards user and feeder
 ...
 */
class SatLeoNetDevice : public NetDevice
{
public:
  /**
  ...
  /**
   * Attach a receive ErrorModel to the SatLeoNetDevice.
  ...
  void ConnectGw (uint32_t gwId, Ptr<SatGwMac> mac);
  void DisconnectGw (uint32_t gwId, Ptr<SatGwMac> mac);
...
private:
  ...
  std::map<uint32_t, Ptr<SatGwMac> > m_connectedGw;
  uint32_t m_activeGw;
};

} // namespace ns3

#endif /* SATELLITE_LEO_NET_DEVICE_H */
